...
const timestampToDateSuccessCases = [
  ...
  },
  {
    ...
    mode: 'auto',
    offset: 0,
    expected: '33658-09-27 01:46:39',
    desc: '邊界案例: 999,999,999,999 (12位，< 1e12 應視為秒)'
  },
  {
    input: 1_000_000_000_000,
    mode: 'auto',
    offset: 0,
    expected: '2001-09-09 01:46:40.000',
    desc: '邊界案例: 1,000,000,000,000 (13位，>= 1e12 應視為毫秒)'
  },
  {
    input: 8_640_000_000_000_000,
    mode: 'ms',
    offset: 0,
    expected: '275760-09-13 00:00:00.000',
    desc: 'JS Date Max Limit (8.64e15 毫秒)'
  }
];
...
const timestampToDateErrorCases = [
  ...
  { input: Number.NaN, mode: 'auto', desc: 'NaN' },
  { input: undefined, mode: 'auto', desc: 'undefined' },
  { input: Symbol('test'), mode: 'auto', desc: 'Symbol (trigger catch)' },
  { input: 8_640_000_000_000_001, mode: 'ms', desc: '超出 Date 範圍 (8.64e15)' }
];
...
describe('timestampToDate', () => {
  describe('成功轉換', () => {
    it.each(timestampToDateSuccessCases)('$desc', (case_: any) => {
      const result = timestampToDate(case_.input, case_.mode, case_.offset);
      expect(result.success).toBe(true);
      expect(result.value).toBe(case_.expected);
    });
  });
  ...
  describe('特殊模式行為', () => {
    it('秒模式: 13位數值應視為秒 (產生遙遠未來)', () => {
      const result = timestampToDate(1_700_000_000_000, 's');
      expect(result.success).toBe(true);
      expect(result.value).toMatch(/^558\d{2}-/);
    });

    it('毫秒模式: 10位數值應視為毫秒 (產生1970年附近)', () => {
      const result = timestampToDate(1_700_000_000, 'ms');
      expect(result.success).toBe(true);
...
describe('dateToTimestamp', () => {
  ...
  describe('毫秒保留', () => {
    it.each(dateToTimestampSuccessCases.filter((c) => c.expectedMs !== undefined))(
      ...
      (case_: any) => {
        ...
        expect(date.getMilliseconds()).toBe(case_.expectedMs);
      }
    );
  });
  ...
  describe('時區轉換 (utcOffset)', () => {
    it.each(dateToTimestampTimezoneCases)('$desc', (case_: any) => {
      const result = dateToTimestamp(case_.input, case_.useMs, case_.offset);
      expect(result.success).toBe(true);
      expect(result.value).toBe(case_.expected);
    });
